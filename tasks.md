...
The live evaluation runner now rebuilds retrieval through `DocumentRegistry` and `RebuildableIndexAdapter` rather than injecting fixture retrieval directly. The pinned `gemini-2.5-flash` revision `001` index-backed rerun returned `0/8` because all eight calls were provider HTTP 429; this is classified as `PROVIDER_LIMITED_REQUIRES_REVIEW`, not a quality score. The current `gemini-3-flash-preview` revision `3-flash-preview-12-2025` index-backed rerun accepted `6/8`, with two provider HTTP 429 failures and zero quality/adapter rejections. The earlier fixture-direct 8/8 run remains bounded to its earlier retrieval path and cannot be treated as runtime-index evidence. Runtime semantic index deployment, persistence ownership/retention, repeated samples, human review and clinical retrieval remain pending; the P2-004 gate remains open.
...
| P2-004 | In Progress | Index-backed Gemini 2.5 rerun is provider-limited at 0/8; Gemini 3 index-backed rerun is 6/8 with two provider HTTP 429; runtime ownership and human review remain open | Repeat pinned Gemini 2.5 after provider window, add repeated samples, finalize persistence/retention ownership and evaluate runtime semantic index |
