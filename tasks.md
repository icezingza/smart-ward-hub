...
เพิ่ม fixture-only conformance evaluator/exporter และ focused/adversarial tests สำหรับ MQTT, WebSocket, Serial และ BLE ใน `p2_002_adapter_conformance.py`, `export_p2_002_adapter_conformance.py`, `test_p2_002_adapter_conformance.py` และ `test_p2_002_adapter_conformance_phase_end_hardening.py`; ผูก tests เข้า `run_all_tests.py` ถัดจาก `test_p2_edge_iot_adapters.py`.
Focused suite ผ่าน 6 tests และ phase-end hardening gate ผ่านครบ: no network/provider/transport/scheduler imports, cross-transport normalized result, failure matrix, exporter round-trip/redaction, no-self-authorization/private-key scan และ `git diff --check`. แก้ evaluator expected failure ของ `source_identity_mismatch` เป็น `source_device_mismatch` ให้ตรงกับ adapter semantics เดิม ซึ่งตรวจ source device mismatch ก่อน transport identity mismatch.
Commit implementation/runner: `2ebdb8dc7e31ce436e858f27ef32eba69a60a5d9`; freeze หลัง implementation `PASS`, source revision `2ebdb8dc7e31ce436e858f27ef32eba69a60a5d9`. สร้าง evidence `evals/micro_rag/evidence/p2-002-adapter-conformance-local.json` ได้ decision `P2_002_ADAPTER_CONFORMANCE_VERIFIED`, `fixture_only=true`, `hardware_evidence=UNVERIFIED`, `redaction_verified=true`, `external_transmission_performed=false`, `production_ready=false`. Snapshot ถูก commit/push แยกแล้ว; ต้อง refresh freeze หลัง snapshot และหลังเอกสารทุกครั้ง.
ขอบเขตยังเป็น software-only internal evidence ไม่ใช่ hardware/driver/broker/RF/reconnect/power-loss/production-network validation และไม่ใช่ HIS/IdP/mTLS, clinical validation หรือ external authorization. External Gates ยังคง `7 BLOCKED / 3 OPEN / 0 PASSED`; authorization boundary และ Product `NOT_PRODUCTION_READY` ไม่เปลี่ยน. รายละเอียดอยู่ใน `P2_002_ADAPTER_CONFORMANCE_READINESS_REPORT_20260822.md`.

### P2-004 Evidence Sufficiency & Provenance Gate — 22 สิงหาคม 2026
เพิ่ม `p2_004_evidence_sufficiency.py`, `export_p2_004_evidence_sufficiency.py`, `test_p2_004_evidence_sufficiency.py` และ `test_p2_004_evidence_sufficiency_phase_end_hardening.py` เพื่อบังคับ sample adequacy, expected case count, provider/runtime/quality blocker separation, DocumentRegistry/RebuildableIndex provenance, release-freeze membership/hash, redaction และ locked no-authorization boundary. ผูก focused และ phase-end tests เข้า `run_all_tests.py` ถัดจาก Micro-RAG registry-backed evaluation tests.
Focused suite ผ่าน 6 cases และ phase-end gate ผ่าน: current one-sample/provider-limited evidence blocked, clean synthetic two-sample path review-ready without authorization, malformed count/status conflict, provenance/raw-identity mutation, freeze/hash/authorization mutation, no network/provider/scheduler imports, exporter round-trip, no-self-authorization/private-key scan และ `git diff --check`.
Current snapshot `evals/micro_rag/evidence/p2-004-evidence-sufficiency-local.json` คืน `P2_004_EVIDENCE_SUFFICIENCY_BLOCKED` อย่างถูกต้อง เพราะ aggregate มี `sample_count=1` จาก `minimum_samples=2` และ `provider_limited_cases=2`; `quality_pass_rate=1.0` จึงถูก suppress จาก review/production claim. `external_review_eligible=false`, `production_ready=false`, `clinical_validation=PENDING`, `runtime_authority=NONE`, `redaction_verified=true`, `freeze_status=PASS` และ `hardware_evidence=UNVERIFIED`.
Feature commit `ccdd80ef1bbadedc1f58df9d2d0224d9b862d62e`; freeze หลัง implementation `PASS` source revision เดียวกัน. Snapshot ถูก commit/push แยกแล้ว; ต้อง refresh freeze หลังเอกสาร, รัน master regression และ final hygiene/alignment ก่อนปิด workstream. การแก้ blocker ต้องใช้ approved non-production provider window เพื่อให้ได้อย่างน้อย 2 compatible samples; repository ไม่สามารถ self-authorize, appoint reviewer หรือสร้าง clinical/production evidence ได้.

### P2-005 Worker Control Consistency Gate — 22 สิงหาคม 2026
เพิ่ม `p2_005_worker_control_consistency.py`, `export_p2_005_worker_control_consistency.py`, `test_p2_005_worker_control_consistency.py` และ `test_p2_005_worker_control_consistency_phase_end_hardening.py` เพื่อรวมการตรวจ semantic consistency ระหว่าง in-process worker control plane, SQLite `software_fixture`, durable replay/backup restore, recovery transcript, operator approval/read-back และ cross-package binding. ผูก tests เข้า `run_all_tests.py` หลัง cross-package binding tests.
Focused suite ผ่าน 6 cases และ phase-end gate ผ่านครบ: allowlisted/idempotent success, stale-lease block/reconcile/recover, durable WAL/FULL integrity, retry/dead-letter/backup restore, transcript/approval/binding integrity, runtime/authorization/redaction mutation fail-closed, no network/provider/scheduler imports, exporter round-trip, private-key scan และ `git diff --check`.
Evidence `evals/micro_rag/evidence/p2-005-worker-control-consistency-local.json` คืน `P2_005_WORKER_CONTROL_CONSISTENCY_VERIFIED`, remediation ว่าง, cross-package binding `BOUND`, `read_only=true`, `runtime_mutation_performed=false`, `external_transmission_performed=false`, `authorization_promoted=false`, `production_ready=false`, `hardware_evidence=UNVERIFIED`. Feature commit `e3d4373a8adc08b0a8a54450357313f6957d4f3b`; freeze หลัง implementation `PASS` source revision เดียวกัน; snapshot ถูก commit/push แล้ว. ต้อง refresh freeze หลังเอกสาร, รัน master regression และ final hygiene/alignment ก่อนปิด workstream.
ขอบเขตยังเป็น deterministic local software evidence เท่านั้น ไม่ยืนยัน distributed worker/scheduler, Windows service recovery, encrypted backup custody, retention/RPO/RTO, external dead-letter delivery, human sign-off, clinical mutation หรือ external authorization. External Gates ยังคง `7 BLOCKED / 3 OPEN / 0 PASSED`; Product ยังคง `NOT_PRODUCTION_READY`.
