...
| External Authorization API handoff | In Progress | Offline simulator validates document lifecycle only; real endpoint, transport, ACL, signed response, expiry/revocation and custody remain unverified | Use `EXTERNAL_AUTHORIZATION_API_SIMULATION_CONTRACT.md` to prepare a real API test plan, then obtain external endpoint and reviewer evidence without changing local authorization flags |
| External Authorization API v2 hardening | In Progress | EA-001–EA-020 gap register identified atomic transition, decision lifecycle, stale polling, uncertain commit, audit integrity and schema gaps; v2 software tests cover the local simulator only | Close remaining Wave A–D software gaps, then run separately approved non-production external API validation with signed response, OIDC/mTLS, ACL, expiry/revocation and independent read-back evidence |
