...
- **P0-004 — In Progress:** a software harness verifies atomic checkpoint restart, corrupt-checkpoint safe behavior and SQLite WAL/synchronous-FULL reopen behavior. Real Acer power cut, disk-full, filesystem corruption and storage-controller recovery remain unverified.
...
| ID | Task | Owner role | Dependencies | Acceptance evidence | Status |
|---|---|---|---|---|---|
| P1-001 | Define encrypted backup/restore and retention | Reliability operator | Backup skill overlay | Manifest, checksum and isolated successful restore transcript | Planned |
| P1-002 | Harden host and least-privilege deployment | Host operator + security auditor | P0 hardware/network observations | OS, firewall, account, patch, disk and service-binding checklist | Planned |
| P1-003 | Define Device Trust key custody/provisioning | Security auditor + registry manager | Device Trust baseline | Manufacturer CA/HSM/secure-element or approved alternative design, rotation and lost-device drill | Planned |
...
| P2-004 | In Progress | Current prompt revision model rerun is blocked by provider HTTP 429; registry/index needs broader corpus and review process | Rerun eight model cases, add repeated samples and bilingual adversarial corpus, then implement approved registry persistence and measured semantic index |
