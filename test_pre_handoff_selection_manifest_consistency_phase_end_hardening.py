"""Phase-end hardening gate for selection-to-manifest consistency."""
...
from export_pre_handoff_selection_manifest_consistency import export_consistency
from pre_handoff_selection_manifest_consistency import check_repository


ROOT = Path(__file__).resolve().parent
TARGETS = (ROOT / "pre_handoff_selection_manifest_consistency.py",)
FOCUSED = ROOT / "test_pre_handoff_selection_manifest_consistency.py"
FORBIDDEN_IMPORTS = {
    "requests",
    "httpx",
    "socket",
...
def run() -> None:
    ...
    assert completed.returncode == 0, f"focused consistency suite failed:\n{completed.stdout}\n{completed.stderr}"
    print("[SelectionManifest GATE] focused/adversarial suite: PASSED")
    ...
    print("[SelectionManifest GATE] no network/provider/scheduler imports: PASSED")

    report = check_repository(ROOT)
    assert report["decision"] == "SELECTION_MANIFEST_CONSISTENT"
    ...
    print("[SelectionManifest GATE] repository consistency and locked output: PASSED")

    with tempfile.TemporaryDirectory() as directory:
        output = Path(directory) / "selection-manifest-consistency.json"
        exported = export_consistency(output=output, project_root=ROOT)
        assert json.loads(output.read_text(encoding="utf-8")) == exported
        assert exported["decision"] == "SELECTION_MANIFEST_CONSISTENT"
    ...
    print("[SelectionManifest GATE] consolidated exporter round-trip: PASSED")
    ...
    print("[SelectionManifest GATE] redaction and no-self-authorization scan: PASSED")

    diff_check = subprocess.run(["git", "diff", "--check"], cwd=ROOT, capture_output=True, text=True)
    assert diff_check.returncode == 0, diff_check.stdout + diff_check.stderr
    print("[SelectionManifest GATE] git diff --check: PASSED")
    print("PRE_HANDOFF_SELECTION_MANIFEST_CONSISTENCY_PHASE_END_HARDENING_GATE_PASSED")
