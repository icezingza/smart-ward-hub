...
เพิ่ม local-only/read-only reviewer read-back control สำหรับ external decision lifecycle โดยใช้ deterministic fixtures เท่านั้น. Focused/adversarial suite 6 cases และ phase-end hardening ผ่านครบ รวม fresh/stale poll, revision/hash lifecycle, external actor separation, local snapshot mutation, appointment/handoff dependency mutation, exporter round-trip, redaction/private-key scan, no network/provider/transport/scheduler imports และ `git diff --check`.
