...
| External Authorization API simulation | `external_authorization_api_simulator.py` v2 models offline submit/poll/finding/audit lifecycle, atomic transitions, expiry/revocation, stale polling, uncertain commit reconciliation, private snapshots and audit fail-stop; current report is simulation-only and authorization flags remain false |
| External Authorization API gap register | `EXTERNAL_AUTHORIZATION_API_FAIL_CLOSED_GAP_REGISTER.md` records 20 gaps EA-001–EA-020; Wave A–D software hardening is in progress and real external validation remains separate |
