...
| R-061 | External reviewer receives metadata-only evidence without a per-test result or failure classification | High | Wave E evidence-v1 schema and bundle control | `test_run_id`, `test_case_id`, expected/actual result and failure class are required; `WaveEEvidenceBundle` enforces exactly one record for every T-01–T-12 and rejects missing/duplicate cases | external owner must provide one record per T-01–T-12 and independent verifier must reproduce/read back the referenced artifacts |
| R-062 | Signed response is described but key, algorithm, signed payload and independent read-back are not bound to the evidence record | Critical | Wave E evidence-v1 schema and bundle control | `signature_ref`, `key_id`, `signature_algorithm`, `signed_payload_hash`, verification result and read-back ref are required for review-ready evidence; bundle rejects any non-review-ready record | real trust chain, key custody, signature verification service and independent second-channel read-back remain external-unverified |
...
| R-066 | Same role can stop, recover, prepare and independently verify a test without separation of duties | Critical | Wave E bundle role-separation control | bundle requires preparer=custodian and distinct external owner, independent verifier, stop authority and recovery approver; record-level stopped/recovery roles must match the declared authority roles | external governance must enforce independent stop, recovery approval and verification identities |
